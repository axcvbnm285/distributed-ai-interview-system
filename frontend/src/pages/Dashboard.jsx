...
function Dashboard() {
  ...
  const createRoom = async () => {
    ...
    setError("");
    try {
      const response = await api.post("/rooms", {}, { headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  const joinRoom = async () => {
    ...
    setError("");
    try {
      await api.post(`/rooms/${roomCode}/join`, {}, { headers: { Authorization: `Bearer ${getToken()}` } });
      navigate(`/room/${roomCode}`);
    } catch {
      setError("Unable to join room. Check the code and try again.");
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-16 animate-fade-in-up">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight mb-4">
            Ready to <span className="shimmer-text">collaborate?</span>
          </h1>
          <p className="text-gray-500 max-w-md mx-auto text-base">
            Create a new interview room or join an existing session with your team.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
          ...
          <div className="glass rounded-2xl p-7 flex flex-col gap-5 animate-fade-in-up delay-100 group hover:border-violet-500/30 transition-all duration-300">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">Start a new interview session and invite candidates or teammates.</p>
            </div>
            <button
              onClick={createRoom}
              disabled={loadingCreate}
              className="mt-auto w-full bg-violet-600 hover:bg-violet-500 disabled:bg-violet-800 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-violet-900/40 hover:shadow-violet-700/40 hover:-translate-y-0.5 active:translate-y-0"
            >
              {loadingCreate ? (
                <>
          ...
          {/* Join Room */}
          <div className="glass rounded-2xl p-7 flex flex-col gap-5 animate-fade-in-up delay-200 group hover:border-violet-500/30 transition-all duration-300">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">Enter a room code to join an ongoing interview session.</p>
            </div>
            <div className="mt-auto space-y-3">
              ...
              <button
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/40 hover:shadow-emerald-700/40 hover:-translate-y-0.5 active:translate-y-0"
              >
                {loadingJoin ? (
                  <>
                    <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
